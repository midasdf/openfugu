        ...
        switch (openfugu.cli.interactiveInput(line)) {
            ...
            .help => {
                try replaceLog(init.gpa, &last_output,
                    ...
                    \\  :plan    preview workflow plan; no arg uses last task
                    ...
                    \\  :stash-patch show latest stash patch
            ...
            .fetch => {
                try runGitFetch(init, &last_output);
                try writer.interface.writeAll(last_output);
            },
            .pull => {
            ...
            .stash_show => {
                try runGitStashShow(init, &last_output);
                try writer.interface.writeAll(last_output);
            },
            .stash_patch => {
                try runGitStashPatch(init, &last_output);
                try writer.interface.writeAll(last_output);
            },
            .stash_pop => {
                try runGitStashPop(init, &last_output);
                try writer.interface.writeAll(last_output);
            },
            .cancel => {
            ...
            .unstage => |path| {
                try runGitUnstage(init, &last_output, path);
                try writer.interface.writeAll(last_output);
            },
            .commit => |message| {
        ...
        for (events) |event| {
            ...
            switch (key.key) {
                ...
                .enter => {
                    ...
                    var plan_task: ?[]u8 = null;
                    defer if (plan_task) |task| init.gpa.free(task);
                    ...
                    switch (openfugu.cli.interactiveInput(line)) {
                        ...
                        .plan_last => {
                            ...
                            plan_task = try std.fmt.allocPrint(init.gpa, ":plan {s}", .{task});
                            try input_history.append(try init.gpa.dupe(u8, std.mem.trim(u8, line, " \t\r\n")));
                            try saveInputHistory(init, &input_history);
                        },
                        .route_last => {
                    ...
                    const should_quit = try handleInteractiveLine(init, plan_task orelse route_task orelse rerun_task orelse line, &last_output, &agents, &history, &dry_run, &agent_filter, &mode, &planner, &term, &job);
                    output_offset = null;
...
fn handleInteractiveLine(
    ...
    switch (openfugu.cli.interactiveInput(line)) {
        ...
        .stash_patch => try runGitStashPatch(init, last_output),
        ...
        .plan_last => try replaceLog(init.gpa, last_output, "no previous task\n"),
...
fn runGitFetch(init: std.process.Init, log: *[]u8) !void {
    try runGitCommand(init, log, ":fetch", &.{ "git", "fetch", "--all", "--prune" }, "fetched\n");
}
...
fn runGitStashShow(init: std.process.Init, log: *[]u8) !void {
    try runGitCommand(init, log, ":stash-show", &.{ "git", "stash", "show", "--stat" }, "no stash entries\n");
}

fn runGitStashPatch(init: std.process.Init, log: *[]u8) !void {
    try runGitCommand(init, log, ":stash-patch", &.{ "git", "stash", "show", "-p" }, "no stash entries\n");
}

fn runGitStashPop(init: std.process.Init, log: *[]u8) !void {
    try runGitCommand(init, log, ":stash-pop", &.{ "git", "stash", "pop" }, "no stash entries\n");
}
...
fn runIssue(init: std.process.Init, log: *[]u8, id: []const u8) !void {
    try runGitCommand(init, log, ":issue", &.{ "gh", "issue", "view", id }, "no issue\n");
}
...
fn runLocalCheck(init: std.process.Init, log: *[]u8) !void {
    try runVerificationCommands(init, log, ":check", &.{
        .{ .name = "build", .argv = &.{ "zig", "build" } },
        .{ .name = "test", .argv = &.{ "zig", "build", "test" } },
        .{ .name = "fmt", .argv = &.{ "zig", "fmt", "--check", "." } },
    });
}
